...
// @desc    Upload Vision Board file
// @route   POST /api/projects/:id/vision-board
// @access  Private (Admin, Performance Marketer)
exports.uploadVisionBoard = async (req, res, next) => {
  try {
    ...
    // Add file to vision board
    project.visionBoard.push({
    ...
    res.status(200).json({
      success: true,
      message: 'Vision board file uploaded successfully',
      data: project.visionBoard[project.visionBoard.length - 1]
    });
  } catch (error) {
    next(error);
  }
...
// @desc    Delete Vision Board file
// @route   DELETE /api/projects/:id/vision-board/:fileId
// @access  Private (Admin, Performance Marketer)
exports.deleteVisionBoardFile = async (req, res, next) => {
  try {
    ...
    // Remove file from vision board
    project.visionBoard = project.visionBoard.filter(
    ...
    res.status(200).json({
      success: true,
      message: 'Vision board file deleted successfully'
    });
  } catch (error) {
    next(error);
  }
...
// @desc    Upload 10-Year Strategy file
// @route   POST /api/projects/:id/ten-year-strategy
// @access  Private (Admin, Performance Marketer)
exports.uploadTenYearStrategy = async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    // Add file to ten year strategy
    project.tenYearStrategy.push({
      fileName: req.file.originalname,
      filePath: req.file.path,
      publicId: req.file.public_id || req.file.filename,
      uploadedBy: req.user._id
    ...
    res.status(200).json({
      success: true,
      message: '10-Year Strategy file uploaded successfully',
      data: project.tenYearStrategy[project.tenYearStrategy.length - 1]
    });
  } catch (error) {
    next(error);
  }
...
// @desc    Delete 10-Year Strategy file
// @route   DELETE /api/projects/:id/ten-year-strategy/:fileId
// @access  Private (Admin, Performance Marketer)
exports.deleteTenYearStrategyFile = async (req, res, next) => {
  try {
    const { id, fileId } = req.params;
    ...
    // Remove file from ten year strategy
    project.tenYearStrategy = project.tenYearStrategy.filter(
      file => file._id.toString() !== fileId
    );
    ...
    res.status(200).json({
      success: true,
      message: '10-Year Strategy file deleted successfully'
    });
  } catch (error) {
    next(error);
  }
